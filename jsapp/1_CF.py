...
# function to load in all the user data and assign it to a dataframefor saved 
...
# computing pairwise similarities that gets the number of common tracks and divides it by the set of tracks between each users
# does this for ever user in database
def compute_pairwise_user_similarity(user_saved_tracks_dict):
    similarities = []
    user_ids = list(user_saved_tracks_dict.keys())
    for user_1, user_2 in combinations(user_ids, 2): # only in sets of 2 at a time
...
# storing the user similarities in the database 
...
# function to build a matrix where rows = users, columns = tracks, values = 1 if track is saved
def build_user_track_matrix(saved_tracks):
    all_users = saved_tracks['user_id'].unique()
    all_tracks = saved_tracks['track_id'].unique()

    # associating each user to the track in a dictionary index
...
# loading all smiliarity scores into a lookup map (for recommendations below)
...
def compute_collaborative_recommendations(matrix, all_users, user_to_index, index_to_track, similarity_map, top_k_user_to_compare):
    ...
    for user_id in all_users:
        ...
        neighbors = sorted(similarity_map.get(user_id, []), key=lambda x: x[1], reverse=True)[:top_k_user_to_compare]
        if not neighbors:
            continue
        ...
        # weighted average of neighbor interactions
        ...
        # skip if all recommendations that are 0
        if np.all(weighted == 0):
            continue

        # get top-N recommended track indices
...
# storing the final recommendations in the database under "Collaborative_Recommendations" table
